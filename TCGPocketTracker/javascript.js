//image sorce https://ptcgpocket.gg/cards/
let cardSet = {
    "A1": {
        "name": "Genetic Apex",
        "set": [
...
for (k in cardSet) {
    ...
    if (cardSet[k].set.length > save[k].save.length) {
        alert("New set info, propigating save")
    ...
    blankSave[k] = {
        'save': new Array(cardSet[k].set.length).fill(0)
    }
...
$(document).ready(function () {
    ...
    $('#container').on('click', '.cardArt', function () {
        $('#cardData').toggle()
    ...
    document.getElementById('importSave').onchange = function () {
...
function calculateRarities() {
    for (k in cardSet) {
        ...
        for (i = 0; i < cardSet[k].set.length; i++) {
            ...
            if (card.pack in pt[k]) {

                pt[k][card.pack].total++
                pt[k][card.pack].rarity[card.rare]++
                if (save[k].save[i] > 0) {
                    pt[k][card.pack].owned++
                }
            }
            /*else if (card.pack === "All") {
                let keys = Object.keys(pt[k]).filter(e => e !== 'All');
                for (l = 0; l < keys.length; l++) {
                    pt[k][keys[l]].total++
                    pt[k][keys[l]].rarity[card.rare]++
    ...
    for (k in cardSet) {
        for (p in cardSet[k].packs) {
            if (k != 'PROMO') {
                ...
                for (r in packT.rarity) {
                    let rareTotal = packT.rarity[r]
                    if (pt[k].All != undefined) {
                        rareTotal += pt[k].All.rarity[r]
                    }
                    rates[packName][r] = NaNtoArray(divideArray(rarities[r], rareTotal))
    ...
    //rates = JSON.parse(JSON.stringify(rates).replaceAll('null', '0'))
                ...
                for (i = 0; i < cardSet[k].set.length; i++) {
                    ...
                    if (cardSave === 0 && (card.pack === pk || card.pack === 'All')) {
                        ...
                        for (l in chancePerCard) {
                            chancePerCard[l] = chancePerCard[l]
                        }
                    }

                }
                pt[k][pk].chancePerCard = chancePerCard
            } else {
                pt[k][pk].chancePerCard = [0, 0, 0, 0, 0]
            }
        }
...
/*
let objInput = ``.split('\n')
//objInput is the last card number of that type, then that type. ec: 5 grass
let obj = {}
for (i=0; i<objInput.length; i++) {
    let split = objInput[i].split(' ')
...
let lastarr = ``.split('\n')
//lastarr is for the higher rarity stuff technically. Basically the fullarts. Put their type in each line

let input = []
//take the whole set from cardSet for input

let typesUsed = []
//get from the type used generator


let type = []
let temp = 0
for (k in obj) {
    let tempArr = Array(k-temp).fill(obj[k])
  temp = k
  type = type.concat(tempArr)
}

  type = type.concat(lastarr)
console.log(type.join('\n'))
for (i = 0; i<type.length; i++) {
    input[i].type = type[i]
  input[i].eUsed = typesUsed[i]
}


$('#out').html(JSON.stringify(input, null, 2))
*/

/*

//Used Types generator. Put it in Jquery, give the input. boom.

let input = ``.split('\n')
let typesUsed = []
for (i=0; i<input.length; i++) {
    typesUsed[i] = input[i].split(',')
}

$('#out').html("let typesUsed = "+JSON.stringify(typesUsed, null, 2))
*/